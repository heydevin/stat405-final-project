...
According to the distributions of our explanatory variables, we notice that those variables all have different ranges. In order to do later on analysis, we need to standardize our 8 predictors to make our prior specifications become consistent. And without standardization, coefficients are not comparable.
```{r}
df <- df %>%
  mutate(
...
$$\alpha \sim \mathcal{N}(16, 2) \\ \beta_1 \sim \mathcal{N}(), \\ \beta_2 \sim \mathcal{N}(), \\ \beta_3 \sim \mathcal{N}(), \\ \beta_4 \sim \mathcal{N}(), \\ \beta_5 \sim \mathcal{N}(), \\ \beta_6 \sim \mathcal{N}(), \\ \beta_7 \sim \mathcal{N}(), \\ \beta_8 \sim \mathcal{N}(), \\ \sigma \sim ()$$
...
$$\log(salary)_i \sim \mathcal{N}(\alpha_j + \beta_1 \cdot USG_i + \beta_2\cdot TO_i + \beta_3\cdot TS_i + \beta_4 \cdot PpG_i + \beta_5 \cdot RpG_i + \beta_6 \cdot ApG_i + \beta_7 \cdot SpG_i +\beta_8 \cdot BpG_i,\ \sigma^2)$$

And we have

$$\alpha_j \sim \mathcal{N}(\mu_a, \tau^2) \\ \beta_1 \sim \mathcal{N}(), \\ \beta_2 \sim \mathcal{N}(), \\ \beta_3 \sim \mathcal{N}(), \\ \beta_4 \sim \mathcal{N}(), \\ \beta_5 \sim \mathcal{N}(), \\ \beta_6 \sim \mathcal{N}(), \\ \beta_7 \sim \mathcal{N}(), \\ \beta_8 \sim \mathcal{N}(), \\ \tau \sim ()$$
